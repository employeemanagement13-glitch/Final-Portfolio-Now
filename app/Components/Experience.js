...
export default function QualificationSection() {
  ...
  return (
    <section id="qualification" className="py-20 px-4 sm:px-6 md:px-10 text-white bg-black">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="w-24 sm:w-32 h-[3px] bg-[#ff2c2c] mx-auto mb-6 rounded-full"></div>
          ...
          <button
            ...
            className={`px-6 py-2 text-sm font-semibold rounded-full transition cursor-pointer 
              ${activeTab === 'experience'
                ? 'bg-[#fd1919] text-white shadow-md'
        ...
        {/* Timeline */}
        <div className="relative border-l border-gray-900 pl-4 sm:pl-6 space-y-10">
          {qualifications[activeTab].map((item, index) => (
            <div key={index} className="relative">
              <span className="absolute -left-4 sm:-left-3 top-1 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#fd1919] flex items-center justify-center text-sm">
                {activeTab === 'experience' ? '✨' : '🎓'}
              </span>

              <div className="bg-[#111111fa] border border-[#222222] rounded-xl p-4 sm:p-6 shadow-lg">
                <h3 className="text-base sm:text-lg font-bold text-white break-words">{item.title}</h3>
                {item.company && (
                  <p className="text-[#fd1919] font-semibold mb-2 break-words">
                    {item.company}
                  </p>
                )}
